...
function getNativeEditorSubmitContext(options = {}) {
  return {
    ...
    draftKey: options.draftKey || getNativeEditorDraftKey(options.editor || document.querySelector(".createTopic, #sendTopicInfo"))
  };
}
...
function releaseNativeEditorForSubmit(editor, context = {}) {
  ...
  editor.dataset.cc98NativeSubmitReleased = "true";
  closeEditorColorPopovers();
  closeEditorFontSizePopovers();
  setEditorEmojiPanelOpen(editor, false);
  removeNativeEditorDraftControls(editor);

  const app = editor.closest("#cc98-comfort-app") || document.querySelector("#cc98-comfort-app");
  restoreNativeNode(editor);
  if (app) {
    legacyQuoteGuardObserver?.disconnect();
    legacyQuoteGuardObserver = null;
    restoreReparentedNativeNodes(app);
    app.remove();
  }
...
function replayNativeEditorSubmitClick(editor, control, context = {}) {
  if (!(editor instanceof HTMLElement)) {
    return false;
  }
  ...
  if (editor.dataset.cc98SubmitTransactionActive === "true" || submitControl.dataset.cc98SubmitReplayQueued === "true") {
    return true;
  }
  const payload = getNativeEditorSubmitContext(context);
  editor.dataset.cc98SubmitTransactionActive = "true";
  submitControl.dataset.cc98SubmitReplayQueued = "true";
  releaseNativeEditorForSubmit(editor, payload);
  schedulePostSubmitPageRefresh(payload);
  window.setTimeout(() => {
    editor.dataset.cc98NativeSubmitReplaying = "true";
    try {
      triggerOriginalControl(submitControl);
    } finally {
      window.setTimeout(() => {
        delete editor.dataset.cc98NativeSubmitReplaying;
      }, 420);
      window.setTimeout(() => {
        delete editor.dataset.cc98SubmitTransactionActive;
        delete submitControl.dataset.cc98SubmitReplayQueued;
      }, 1800);
    }
  }, 0);
  return true;
...
function schedulePostSubmitPageRefresh(options = {}) {
  ...
  nativePostSubmitRefreshTimers = [180, 420, 900, 1000, 1600, 2600, 4200, 6500, 9000].map((delay, index, delays) => {
    return window.setTimeout(() => {
      ...
      if (getTopicRouteInfo()) {
        const routeChangedToTopic = !startedOnTopic && getRoutePageKey() !== startedRouteKey;
        const safeExistingTopicRefresh = startedOnTopic && delay >= 3200;
...
function bindNativeEditorStabilizer(editor) {
  ...
  editor.addEventListener("click", (event) => {
    ...
    if (isNativePostSubmitEditor(editor) && isLikelyEditorSubmitControl(event.target)) {
      ...
      event.preventDefault();
      event.stopImmediatePropagation();
      replayNativeEditorSubmitClick(editor, submitControl, context);
      return;
    }
