...
- A non-interactive OpenID authorization attempt about once per hour after binding, used only to re-read `/me` and refresh that local identity summary.
...
OpenID binding does not persist access tokens or refresh tokens. A short-lived token is used to request `https://api.cc98.org/me`; if that endpoint temporarily returns a server error, the extension may request `https://openid.cc98.org/connect/userinfo` as a basic-profile fallback. The token is then discarded. When CC98 is accessed through WebVPN, authorization may use a WebVPN tab and a local callback bridge; the bridge reports only the OpenID callback URL to the extension background and does not send forum page content elsewhere.
