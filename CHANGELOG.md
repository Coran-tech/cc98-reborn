...
- 发帖、回复、编辑及签名档的双区 UBB 工具栏加入“旧预览”入口，可随时打开以 `0.3.1` 为基线的只读预览作为备用；颜色、基础格式、对齐、媒体与本地表情均会按当前源码重新渲染。
- 双区 UBB 工具栏加入“同步”按钮：以最近操作的上区或下区为准，主动写回原站文本域，并重新渲染上下区域及已打开的旧版预览，便于在实时互译状态异常时手动校准。

### 修复

- 重做帖子楼层锚点恢复：首次打开带 `#楼层` 的链接会等待目标卡片就绪后定位，并兼容分页页面中的绝对楼层号与页内楼层号；页面自动同步或重建不再把阅读位置反复拉回页首，用户主动滚动后也不会被延迟定位打断。
- 修复双区编辑器上下区域纵向拉伸时，外层页面高度与中间分隔线没有随之移动的问题；分隔线拖动与键盘微调现在会同步校正两区高度和文档流。
- 修复下区占位提示被当作真实文本、源码换行未在上区正确显示，以及空白内容中的辅助字符可能进入 UBB 源码的问题。
- 加强上区选区到 UBB 源码的映射，修复第一行行首选中文字着色时选区丢失、生成 `test[color=#ff0000][/color]` 空标签，以及透明色未正确包裹选区的问题。
- 修复删除已着色文字后继续输入仍继承旧颜色的问题，并加入“取消颜色”入口；下划线和删除线的装饰颜色现在会跟随实际文字颜色，而不是误用主题色。
- 修复表情选择面板打开后迅速消失的点击竞争问题；保持上区表情以图片渲染并能回写原 UBB，同时稳定“第一行行首仅有一个表情”时编辑区的外边距与内边距。
- 双区编辑器工具栏在原站 React 重绘后会自动补回“旧预览”和“同步”入口，且避免重复生成按钮。

### 调整

- 增加完整脚本加载的本地 UBB 编辑器回归页与静态测试服务器，覆盖上下区同步、逻辑换行、旧版预览、表情浮窗和首行单表情布局。
- 版本号提升到 `0.3.3`，README、发布检查目标和打包路径同步更新。

## 0.3.2 - 2026-08-06

### 新增

- 双区 UBB 编辑器的源码区加入逻辑行号：只对真实回车产生的新行编号，因容器宽度自然折行的内容不会重复编号；当前行会高亮，滚动、输入和纵向拉伸时同步更新。
- 插件弹窗加入“预热帖子图片”开关并默认关闭；开启后可继续调整预热时长，关闭时相应时长控件会禁用并降低强调。
- 插件弹窗加入“帖子卡片默认展开（关闭悬浮动画）”选项并默认关闭；启用后主题卡片附加信息常驻展开，关闭后保留原有悬浮展开效果。
- 新帖、关注、精选和搜索结果的“稍后再看”侧栏下方加入等宽“▲ 回到顶部”组件，窄屏布局中仍紧跟稍后再看区域。

### 修复

- 修复进入新帖页面时原站帖子尚未就绪、半成品 DOM 就被纳入重构的问题；现在会按路由提前识别新帖页，等待原站内容或空结果信号，并在等待期间定时复查。
- 加强首页与新帖等路由切换时的加载幕布；WebVPN 首页头像预热期间使用不透明遮罩，减少原版页面或未完成内容短暂露出。
- 修复双区编辑器中右对齐行末表情被错误挤到下一行的问题，同时避免行内表情容器跨越对齐块继续扩张。
- 修复双区编辑器空白提示不一致的问题：上区统一为“在这里直接编辑并预览内容”，下区统一为“在这里以经典代码形式编辑内容”。
- 修复切换到 Markdown 模式后原双区 UBB 编辑器仍然显示的问题；切换时会拆除旧双区实例并强制隐藏残留外壳，切回 UBB 后再正常重建。
- 帖子页底部回复组件宽度调整为与正文卡片一致，避免回复区明显偏窄。
- “我的消息”页面标题区改为与消息主体使用同一居中内容轴，修复标题过度靠左的问题。
- 稳定顶部导航栏的最小高度、网格行和动作区宽度，并关闭滚动锚定干扰，减少页面加载期间的上下抖动。
- 重新排版个人中心“修改资料 → 其他设置”：标签与控件对齐，下拉框文字居中并限制宽度，生日、头衔、简介等字段在桌面与窄屏下使用更紧凑的布局。
- 修复顶部用户菜单“签到”入口在直连与 WebVPN 下跳转不稳定的问题，统一交由重构后的站内路由处理。
- 修复签到页按钮被误判为发帖提交、双区编辑器干扰原站签到状态更新的问题；签到页不再接入草稿和发帖事务，隐藏无必要的签到 UBB 输入框，并将连续签到天数、上次签到时间与财富值奖励集中居中显示。

### 调整

- 帖子内图片预热改为完全按需启用；未开启或中途关闭时，不再启动原页面滚动预热，并会取消尚未执行的兜底任务。
- 个人中心原有“回到顶部”按钮加高并加粗，箭头改为实心三角形，图标与文字作为一个整体居中。
- 顶部导航栏原“刷新”入口改名为“同步”，并与旁边的“搜索”按钮统一结构、宽度规则和视觉样式；同步过程仍会显示状态反馈。
- 版本号提升到 `0.3.2`，README、发布检查目标和打包路径同步更新。

## 0.3.1 - 2026-08-05

### 新增

- 签名档编辑器改为上下双区 UBB 编辑：上方可直接编辑渲染后的内容，下方可编辑带结构高亮的 UBB 源码，两边会实时互译并同步到原站字段。
- 将双区 UBB 编辑移植到发帖、回复、编辑主题和编辑回复页面，同时保留原生文本域作为上传、草稿和提交的数据桥。
- 上下编辑区均可独立纵向拉伸；中间分隔线支持鼠标拖动和方向键微调。

### 修复

- 修复输入法组合输入结束、UBB/Markdown 切换时最后一段文字可能延迟同步或选错原生文本域的问题。
- 修复颜色面板选择“透明”时没有包裹当前活动编辑区选中文字的问题；上下编辑区现在都会生成正确的 `[color=transparent]...[/color]`。
- 为源码结构色加入无色文字与光标边界，避免在 UBB 标签旁输入时继承标签高亮色；辅助边界不会进入实际 UBB 内容。
- 修复下方源码区按 Enter 生成浏览器内部节点、却没有写入真实换行的问题；签名档、发帖、回复和编辑页面均统一写入 `\n`。
- 修复顶部“刷新”在内容同步后仍持续显示“同步中”的反馈状态。

### 调整

- 双区编辑器提示统一为“上下都可以编辑哦~”。
- 签名档编辑器停用表情选择器，避免本地表情节点无法稳定回译为原站签名 UBB。
- 版本号提升到 `0.3.1`，发布说明和打包路径同步更新。

## 0.3.0 - 2026-08-03

### 新增

- 新帖、关注、精选及搜索结果页面完善右侧工具栏：稍后再看支持组件内滚动、搜索、翻页和本地同步，搜索排序改为独立侧栏并默认保留原站顺序。
- 搜索联想面板加入按用户 ID 隔离的本地搜索历史，可逐条删除或一键清空。
- 编辑器颜色面板加入“透明”快捷选项，生成 `[color=transparent]...[/color]`，并修复色号输入框无法稳定输入的问题。
- 新帖页面接入原站经典、卡片和只看媒体模式；卡片与媒体模式会同步原页面已经加载的图片。
- OpenID 强制绑定遮罩加入刷新入口，弹窗使用标签展示已绑定账号、UID 与绑定时间。

### 修复

- 全盘收紧原站控件代理与状态同步，改善发帖、回帖、编辑、收藏、评分和分享链接等操作的响应与完成后刷新。
- 修复公开用户详情页频繁重建导致文字选区、顶部搜索类型菜单和用户组件闪动的问题；WebVPN 下已移动进重构界面的用户资料也能被正确识别为加载完成。
- 修复左侧效率栏下回复区、头像层级、悬浮资料卡和用户菜单的遮挡、偏移及收回时机问题。
- 修复部分头像、帖子媒体和新帖卡片占位图的识别问题；不存在真实媒体时不再生成伪图片。
- 修复 OpenID 本地身份摘要与界面说明不一致的问题。定时自动校准保持停用，绑定后直接使用首次授权保存的数据。
- 返回主页、进入用户详情或个人中心时加入短时主题幕布，遮住原版界面闪现；新 UI 就绪后淡出，并带硬性超时兜底。
- 修复部分带用户头衔的详情页中，搜索类型下拉菜单反复消失或闪动的问题。

### 调整

- 页面“刷新”改为读取原页面当前已加载内容并同步到重构 UI；支持页面滚动时无感增量同步。
- 新帖页面每次进入时额外执行一次内容同步，减少初次展示落后于原页面的情况。
- 版面“发主题”入口放大并使用黄色强调；相关布局与左侧效率栏的正文空间同步调整。
- 回复小尾巴继续默认开启，但在匿名回复和心灵之约版面自动停用；弹窗中不再标记为实验性。
- `README.md` 完整改写为中文主文档并内置英文摘要，不再维护独立的 `README.zh-CN.md`。
- 版本号提升到 `0.3.0`，发布文档与隐私说明同步为当前实际 OpenID 行为。

## 0.2.11.1 - 2026-07-30

### 新增

- 帖子作者头像新增悬浮资料卡，集中显示用户名、性别、帖数、粉丝、威望和最后登录等当前页面已有信息，并提供关注与私信入口。
- 版面列表新增锁定主题提示：标题前显示 `🔒`，卡片使用更清晰的淡红色警戒斜纹区分锁帖。

### 修复

- 修复插件黑名单页面在 WebVPN 下仍跳转到直连地址的问题；内部页面现在会沿用当前 WebVPN 页面及其动态编码前缀。
- 修复帖子作者悬浮资料卡头像不显示的问题，加入 WebVPN 资源地址归一化、积极加载和背景图兜底。
- 修复悬浮头像上方错误显示完整用户主页 URL 的问题；头像仍可正常点击进入用户主页。

### 调整

- 本次新增的锁帖识别、作者资料卡、关注和私信入口均同时适配 CC98 直连与 WebVPN；关注继续触发原站按钮，私信和用户主页继续使用原站路由。
- 版本号提升到 `0.2.11.1`。
- 重新生成发布包 `dist/cc98-reborn-0.2.11.1.zip`。

## 0.2.11 - 2026-07-30

### 新增

- OpenID 绑定新增 WebVPN 授权通道：在 WebVPN 标签页中完成授权后，由独立桥接脚本和后台导航监听恢复授权结果，继续完成 PKCE 换票、`/me` 读取及当前网页账号 UID 一致性校验。
- 已绑定的 OpenID 身份摘要和水印信息会约每小时自动校准一次。校准使用非交互授权并在完成后丢弃临时令牌；失败时保留原有本地摘要，弹窗会显示最近校准时间或错误。

### 修复

- WebVPN 下的 OpenID 授权页、登录中心和回调页不再注入论坛 UI 重构与提交监听，避免原站 React 控件被移动后出现 `removeChild` 异常或授权按钮失效。
- 修复 WebVPN 退出登录后，编码路径中的 `/logOn` 未被识别为登录页，导致退出兜底定时器反复刷新登录页的问题。
- 修复 WebVPN 登录成功后停留在编码路径 `/null` 的问题；完整导航和站内路由变化都会立即转回同一 WebVPN 前缀下的 CC98 首页。
- 加强 WebVPN OpenID 回调恢复：授权状态与 PKCE 临时数据可由后台短期保存，并通过标签页 URL、导航事件和页面桥接多路捕获回调，减少授权标签页切换或后台休眠造成的绑定失败。

### 调整

- 补充 `openid-webvpn-bridge.js`、`webNavigation` 权限、每小时身份校准及 WebVPN OpenID 流程的发布文档和语法检查。
- 版本号提升到 `0.2.11`。
- 重新生成发布包 `dist/cc98-reborn-0.2.11.zip`。

## 0.2.10.1 - 2026-07-28

### 修复

- 修复部分特殊用户详情页在选择文字时被延迟重建或增量同步打断、导致选区消失的问题；选取与拖拽期间会暂停用户页重建，取消选区后再补一次同步。
- 统一用户详情页头像尺寸为 `160 × 160px`，使用圆形居中裁切，避免特殊用户头像受原图比例或内联样式影响而大小不一。
- 修复引用块链接指向其他主题时，仅按楼层号在当前帖子内跳转的问题；现在会严格比较目标主题 ID，跨主题引用会前往正确帖子，同主题引用才进行页内楼层定位。

### 调整

- 版本号提升到 `0.2.10.1`。
- 重新生成发布包 `dist/cc98-reborn-0.2.10.1.zip`。

## 0.2.10 - 2026-07-18

### 新增

- 在新帖、关注和精选列表右侧加入竖向“稍后再看”侧栏。侧栏随页面滚动保持悬浮，并支持组件内部滚动、关键词搜索、翻页、标记已读或未读、删除、清除已读以及进入完整列表。
- 右侧栏、新帖卡片上的稍后再看按钮和独立稍后再看页面复用同一份按 CC98 用户 ID 隔离的本地数据；任一入口发生变化后，其余入口会即时同步。
- 插件弹窗的“关于”面板加入 Bug 反馈入口，可直接前往项目 Issues 页面。

### 修复

- 修复 WebVPN 下外部链接被错误套用 CC98 代理路径的问题；GitHub 等非 CC98 链接会直接打开原始地址。
- 修复 WebVPN 首页头像偶发未加载、用户详情页加载较慢时遮罩不收起或页面空白的问题，并恢复用户详情页内容、图片和后续动态的持续加载。
- 修复 WebVPN 用户详情页右上角长期显示“个人中心”占位符的问题。现在会在原站头像和用户名加载完整后生成稳定用户组件，同时保留铃铛、头像、用户名和菜单。
- 修复公开用户详情页资料区偏左的问题；标题与资料卡现在使用一致的居中内容轴。
- 回复小尾巴在心灵之约版面和匿名回复场景中不再插入，避免破坏匿名语境。
- 继续收紧用户中心加载遮罩和用户悬浮菜单的生命周期，减少遮罩残留、菜单抽动及鼠标仍位于菜单内时意外消失的问题。

### 调整

- 新帖、关注和精选的主题列表采用统一双栏布局；主题卡片左端与页面标题左端对齐，窄屏和空间不足时侧栏自动移到列表上方。
- 稍后再看按钮改为本地即时更新，不再仅为刷新按钮状态而重建整个页面。
- 版本号提升到 `0.2.10`。
- 重新生成发布包 `dist/cc98-reborn-0.2.10.zip`。

## 0.2.9 - 2026-07-15

### 新增

- 重新适配原站 Markdown 编辑器，统一编辑区、工具栏、下拉菜单和预览区的明暗主题外观，同时保留原站 Markdown 编辑与提交逻辑。
- Markdown 预览加入本地过渡层：点击预览后先立即渲染编辑区当前可见内容，待输入法组词完成并稳定后，再由原版完整预览无缝接管。
- “切换到 Markdown 编辑器”按钮增加黄色“（实验性）”标记；插件弹窗中的“左侧效率栏”和“回复时添加小尾巴”同步标注为实验性功能。

### 修复

- 修复中文输入法输入词块后立刻切换预览时，最后一次输入可能延迟出现或不同步的问题。
- 修复 Markdown 编辑器切换按钮被挤到提示文字下一行的问题；现在与提示同行并靠右显示，切换到 UBB 时不显示实验性标记。
- 修复右上角用户浮窗在鼠标仍位于菜单中时偶发消失、反复闪动的问题。
- 修复菜单项获得焦点后，鼠标离开菜单外边框仍无法自动收回的问题；现在离开头像与菜单区域约 360ms 后关闭，重新移入会取消关闭。
- 修复左侧效率栏模式下用户菜单与头像重叠、向错误方向展开或超出页面的问题。
- 修复 `/usercenter` 页面缺失原用户入口时，仅显示风格不一致的“个人”占位文字的问题。

### 调整

- 顶部用户菜单改为头像、用户名头部加两列入口，退出登录独立置底并使用红色强调；左侧效率栏使用独立向右展开的单列长条菜单。
- 顶部“刷新”按钮改为重新读取原页面当前已经加载的内容并应用到重构 UI，不再默认触发整页网络刷新。
- 版本号提升到 `0.2.9`。
- 重新生成发布包 `dist/cc98-reborn-0.2.9.zip`。

## 0.2.8.6 - 2026-07-11

### 新增

- Markdown 格式回复改为独立内容框：移除正文文字底色，在左上角断开的框线上标注“Markdown”，同时保留代码块自身的必要底色。
- WebVPN 帖子页加入图片积极预热：识别原站懒加载图片与“点击查看图片”控件，并在重构前触发加载。

### 修复

- 修复 WebVPN 下帖子图片没有进入原页面预热流程的问题，并为较慢的代理页面设置独立的预热与超时节奏。
- 修复自动滚动加载图片时原页面短暂露出的问题；滚动期间遮罩会持续保持，完成、超时或异常后自动释放。
- 回复小尾巴改为默认开启，同时仍可在插件弹窗中关闭。
- 修正发布文档中残留的休眠 OAuth、水印桥接文件和过期版本号说明；发布检查现在覆盖实际运行的 `src/page-submit-monitor.js`。

### 调整

- 版本号提升到 `0.2.8.6`。
- 重新生成发布包 `dist/cc98-reborn-0.2.8.6.zip`。

## 0.2.8.5 - 2026-07-06

### 新增

- 顶部用户入口改为自建合并浮窗：消息入口与个人入口整合为同一个两列菜单，支持普通顶部栏模式和左侧效率栏模式。

### 修复

- 修复首页小铃铛在原站异步加载时偶发缺失的问题：仅在首页缺失时搬回原站已有消息入口，不再创建额外铃铛，也不影响其它页面。
- 修复首页用户入口浮窗寿命过短、鼠标移入菜单时闪动或消失的问题，减少重复重建并加入透明 hover 桥。
- 修复左侧效率栏模式下消息菜单与个人菜单同时弹出、互相遮挡的问题。
- 修复页码导航当前页指示不明显的问题，保留原圆角方块尺寸，仅强化当前页数字与主题色边框。

### 调整

- 版本号提升到 `0.2.8.5`。
- 重新生成发布包 `dist/cc98-reborn-0.2.8.5.zip`。

## 0.2.8.4 - 2026-07-05

### 新增

- 新增“回复时添加小尾巴”可选项。开启后，发表回复时会在正文末尾自动追加一行居右的“来自 CC98 Reborn”彩色链接签名，链接指向项目介绍帖。

### 修复

- 修复回复提交前小尾巴偶发没有真正写入原站输入框的问题，改为在回复提交按钮的 `pointerdown`、`click` 和 `submit` 三个阶段兜底写入。
- 修复提交成功后地址栏已经到达目标帖子页，但遮罩仍停留在“提交已成功，正在刷新帖子...”并持续转圈的问题。
- 去除回复小尾巴末尾多余的中文逗号。

### 调整

- 版本号提升到 `0.2.8.4`。
- 重新生成发布包 `dist/cc98-reborn-0.2.8.4.zip`。
